...
static int func_panic(lua_State *L)
{
	const char *err = luaL_checkstring(L, -1);
	std::string last_error = std::string(err);
	lua_pop(L, 1);
	printf("LUA PANIC: unprotected error in call to Lua API (%s)\n", err);
	//PluginManager::Get()->UnloadPlugin()
	throw 0;
...
class LuaGlue : public LuaGlueBase
{
	public:
		...
		LuaGlue &open()
		{
			...
			luaopen_luamylib(state_);
			lua_atpanic(state_, &func_panic);
			return *this;
		}
